...
case class EventData(eventType: String, timestamp: String, errorData: String,
  ...
  def eventDate(): String = {
    LocalDateTime.parse(timestamp).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))
  }

  def eventId(): String = {
    s"${actualDepTime}-${airTime}-${CRSArrTime}-${carrierCode}-${flightNumber}-${tailNumber}-${origin}-${destination}-${distance}"
  }

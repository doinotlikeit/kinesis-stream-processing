...
object EventDataHandler extends Serializable {
  ...
  def process(eventDataRecord: String, logger: LambdaLogger): EventData = {
    ...
    try {
      ...
      case e: Throwable => {
        logger.log(s"Couldn't process the event data record rcvd: ${e.toString}")
        createErrorEventDataInstance(e.toString)
      }
    }
    logger.log(s"==> Created an EventData object")
    eventData
  }
}
